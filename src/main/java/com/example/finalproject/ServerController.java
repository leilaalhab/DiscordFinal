...
public class ServerController implements Initializable {
    ...
    public void openChannel(String channelIndex) throws IOException {
        ...
        for (String message : messages) {
            ...
            String chatUsername = userString.substring(userString.indexOf("]"), userString.indexOf(":"));
            ((Label)((VBox)pane.getChildren().get(1)).getChildren().get(0)).setText(messageIndex);

            // change to channel name
            String laughs = client.getLaughs(messageIndex, channelIndex);
            String likes = client.getLikes(messageIndex, channelIndex);
            String dislikes = client.getDislikes(messageIndex, channelIndex);
